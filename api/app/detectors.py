...
RULES: list[DetectionRule] = [
    ...
    DetectionRule(
        ...
        type=FindingType.secret,
        pattern=_compile(r"\bsk-(?!ant-)(?:proj-)?[A-Za-z0-9_-]{20,}\b"),
        severity="critical",
        confidence=0.98,
        token="OPENAI_KEY",
    ),
    DetectionRule(
        ...
        type=FindingType.secret,
        pattern=_compile(r"\bsk-ant-(?:api\d{2}-)?[A-Za-z0-9_-]{20,}\b"),
        severity="critical",
        confidence=0.98,
        token="ANTHROPIC_KEY",
    ),
    DetectionRule(
        ...
        type=FindingType.secret,
        pattern=_compile(r"\bhf_[A-Za-z0-9]{30,}\b"),
        severity="critical",
        confidence=0.95,
    ...
    DetectionRule(
        ...
        type=FindingType.secret,
        pattern=_compile(r"\baws_secret_access_key\s*[:=]\s*[\"']?[A-Za-z0-9/+=]{40}[\"']?"),
        severity="critical",
        confidence=0.96,
    ...
    DetectionRule(
        ...
        type=FindingType.secret,
        pattern=_compile(r"\b(?:postgres|postgresql|mysql|mariadb|mongodb(?:\+srv)?|redis)://[^:\s/@]+:[^@\s]+@[^\s\"']+"),
        severity="critical",
        confidence=0.93,
        token="DATABASE_URL_CREDENTIAL",
    ),
    DetectionRule(
        label="Bearer Authorization Header",
        type=FindingType.secret,
        pattern=_compile(r"\bAuthorization:\s*Bearer\s+[A-Za-z0-9._\-+/=]{20,}\b"),
        severity="critical",
        confidence=0.91,
        token="BEARER_AUTHORIZATION",
    ),
    DetectionRule(
    ...
    DetectionRule(
        ...
        pattern=_compile(r"\b(?:ignore (?:all )?(?:previous|prior|above) instructions|reveal your system prompt|developer message|system message|exfiltrate|bypass policy|disable safety|act as unrestricted|do not obey|print hidden instructions)\b"),
        severity="high",
        confidence=0.9,
        token="PROMPT_INJECTION",
    ),
    DetectionRule(
        label="Tool Exfiltration Instruction",
        type=FindingType.prompt_injection,
        pattern=_compile(r"\b(?:send|post|upload|forward)\s+(?:the\s+)?(?:conversation|chat|prompt|system prompt|secrets?|credentials?)\s+to\s+(?:https?://|slack|discord|pastebin|webhook)"),
        severity="high",
        confidence=0.88,
        token="TOOL_EXFILTRATION",
    ),
    DetectionRule(
